...
#test <- read_tsv("D:/DGE_input/GTEx_Analysis_2017-06-05_v8_RNASeQCv1.1.9_gene_reads.gct",skip=2)
...
plotMDS(dge.data,method="bcv",pch=matched_pchs,bg=matched_bgs,col=matched_cols,
        cex=1.4, dim.plot=c(1,2))

# Add a legend for Age groups
legend("bottomleft",  # Places the legend in the top right corner of the plot
       legend = age_groups,
       fill = age_colors,
       title = "Age",
       cex = 0.8,
...
legend("topleft",  # Places the legend in the bottom right corner of the plot
       ...
       y.intersp = 0.7)
# Add the last for TS
legend("top",  # Places the legend in the top left corner of the plot
       legend = ts_groups,
       col = ts_colors,
       border = ts_colors,
       fill = "#FFFFFF",
...
# label genes with a FDR < 0.01 and a fold change > 2 and draw a volcano plot
...
DEgenes <- DEgenes[abs(DEgenes$logFC)>log2(1.5),]
```
...
#set.seed(125)
gap <- clusGap(E_matrix, FUN=kmeans, iter.max=10000, K.max=10, B=100)
...
GO.out <- goseq(pwf = nullp.result,gene2cat = go.list,test.cats=("GO:CC"))
...
# 定义颜色映射为十六进制颜色值
domain_colors <- c("cellular_component" = "#FF7F0E",  # 蓝色的十六进制代码
                   "biological_process" = "#2CA02C")  # 绿色的十六进制代码
...
treemap(GO.out,
        ...
        palette = domain_colors  # 应用颜色映射
)
#1F77B4", "#FF7F0E", "#2CA02C
library(ggplotify)
library(treemapify)
# 准备数据
GO.out$size <- abs(GO.out$logPvalue)  # 可以使用绝对值的logPvalue作为大小指标

# 绘制矩形树图
p <- ggplot(GO.out, aes(area = size, fill = GO_domains, label = category)) +
  geom_treemap() +
  geom_treemap_text(fontface = "italic", colour = "white", place = "centre", grow = TRUE) +
  scale_fill_manual(values = c("cellular_component" = "#1F77B4", "biological_process" = "#2CA02C","molecular_function"="#FF7F0E")) +
  theme(legend.title = element_text(face = "bold"), legend.position = "right") +
  labs(fill = "GO Domains")
...
ego <- enrichGO(gene          = sig_entrez_ids,
                ...
                OrgDb         = org.Hs.eg.db,
                keyType       = "ENTREZID",
                ont           = "ALL",  # "BP" for Biological Process; use "CC" for Cellular Component, "MF" for Molecular Function, "ALL" for all
                pAdjustMethod = "BH",
                qvalueCutoff  = 1,
                pvalueCutoff = 1,
