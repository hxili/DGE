...
# For our subsequent analyses we want to reduce the data set to only those genes with some expression. In this case we will retain genes with > 10 reads in >= 100 samples
counts.data <- counts.data[rowSums(counts.data[,-1] > 10) >= 100,]
...
# Identify the DEGs based on the significance criteria established (e.g., 
# FDR < 0.05 and an absolute LFC greater than log2(1.5)).


degs <- results[results$significant,]

# Retrieve counts data of degs
deg.expr.matrix <- counts.matrix[rownames(counts.matrix) %in% rownames(degs), ]

v <- voom(deg.expr.matrix, design, plot=TRUE)
expr.matrix <- v$E
