...
mds <- plotMDS(dge.data,method="logFC",pch=matched_pchs,bg=matched_bgs,col=matched_cols,
...
colnames(voom_E) <- c(voom_E[1,1:333],1,2,3,4,"group")
voom_E <- voom_E[-1,-334:-337] %>% 
...
set.seed(125)
gap <- clusGap(E_matrix, FUN = kmeans, iter.max = 30, K.max = 30, B = 200)
...
clus <- clus %>% pivot_longer(c(-cluster, -gene_index), names_to = "sample_ID", values_to = "expression") %>% # get ready for plotting
