...
tiss1.sampid <- samp.anno.data %>%
  filter(SMTSD==tissue1) %>% 
  select(SAMPID)

tiss2.sampid <- samp.anno.data %>% 
  filter(SMTSD==tissue2) %>% 
  select(SAMPID)
...
plot_info <- sample.description %>%
  mutate(
    SEX = ifelse(is.na(SEX), "Unknown", SEX),  # Handle missing SEX
    SEX = factor(SEX, levels = c(1, 2, "Unknown"), labels = c("Male", "Female", "Unknown")),
    TS = factor(TS),
...
#colors_tissue <- RColorBrewer::brewer.pal(n = length(unique(plot_info$TS)), name = "Set3")
colors_tissue <- colorRampPalette(RColorBrewer::brewer.pal(9, "Set3"))(length(unique(plot_info$TS)))
...
plot_info <- plot_info %>%
  mutate(
    bg = colors_tissue[TS], # Tissue colors
    pch = ifelse(SEX == "Male", 21, ifelse(SEX == "Female", 24, 22))  # Shape: default for unknown
  )
...
legend("bottomright", inset=c(-0.2, 0), title="Tissue", legend=levels(plot_info$TS), fill=unique(plot_info$bg), pch=21, cex=0.6, y.intersp=0.7, border=unique(plot_info$bg), xpd=TRUE)
```
...
results$significant <- results$FDR < 0.01 & abs(results$logFC) > log2(1.5)
...
ggplot(results, aes(x = logFC, y = -log10(PValue), col=significant)) +
  ...
  geom_hline(yintercept = -log10(0.01),linetype = "dashed",color = "blue") +  # FDR lin
  theme_minimal() +
  labs(x="Log2 Fold Change", y="-Log10 Adjusted P-value", title = "Volcano Plot")
# Summarize the number of differentially expressed genes
summary(decideTestsDGE(ts.lrt,p.value=0.01,lfc=log2(1.5)))
...
DEgenes <- DEgenes[abs(DEgenes$logFC)>1,]
...
domain_colors <- setNames(brewer.pal(length(unique(GO.out$GO_domains)), "Set3"), unique(GO.out$GO_domains))
...
treemap(GO.out,
        ...
        vColor = "GO_domains",  # Assign color based on GO domains
        title = "Over-represented GO Terms in DE Genes",
        fontsize.title = 14,
        palette = domain_colors,  # Custom color palette for GO domains
        border.col = "black",  # Border color
        fontcolor.labels = "black",  # Font color for labels for better visibility
...
treemap(GO.out,
        ...
        vSize = "logPvalue",  # Value determining the size of the rectangles
        vColor = "GO_domains",  # Assign color based on GO domains
        title = "Down-represented GO Terms in Up-regulated Genes",
        fontsize.title = 14,
        palette = domain_colors,  # Custom color palette for GO domains
        border.col = "black",  # Border color
        fontcolor.labels = "black",  # Font color for labels for better visibility
