...
par(mar=c(5, 4, 4, 12) + 0.1)  # Default is c(5, 4, 4, 2) + 0.1
...
legend("bottomright", inset=c(-0.45, 0), title="Tissue", legend=levels(plot_info$TS), fill=unique(plot_info$bg), pch=21, cex=0.6, y.intersp=0.7, border=unique(plot_info$bg), xpd=TRUE)
```
...
ts.lrt <- glmLRT(fit,coef="TSHeart - Left Ventricle")
...
treemap(GO.out,
        index = "category",  # GO term identifiers
        vSize = "logPvalue",  # Value determining the size of the rectangles
        vColor = "lGO_domains",  # Assign color based on GO domains
