...
sample.description <- sample.description %>%
  mutate(
    ...
    AGE = factor(AGE, levels = unique(AGE)),  # just let the unique order as is
...
legend("bottomright", inset=c(-0.2, 0), title="Age", legend=levels(plot_info$AGE), fill=unique(plot_info$bg), cex=0.8, y.intersp=0.7, xpd=TRUE)
...
legend("topright", inset=c(-0.2, 0.2), title="Tissue", legend=levels(plot_info$TS), fill=unique(plot_info$col), pch=21, cex=0.8, y.intersp=0.7, border=unique(plot_info$col), xpd=TRUE)
```
